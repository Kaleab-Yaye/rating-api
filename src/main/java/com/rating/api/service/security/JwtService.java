...
public class JwtService {
    @Value("${jwt.secret-key}") // this is the secret key that will be used with the hashing of the singiture
   ...
   // this will generate a token from the user detials
    public String generateToken(UserDetails userDetails) {
        return Jwts.builder() // no extra claims with this one
                ...
                .expiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24)) // 24 Hours, you can change it
                .signWith(getSignInKey(), Jwts.SIG.HS256) // this is where it will mix our key and all the string information and creat the hash
                .compact();
    }
    ...
    // this is where the raw characher become an array of bits so that all the hashin alcoirhms can do the job.
